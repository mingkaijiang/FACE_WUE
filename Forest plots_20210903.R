forest_plots <- function(inDF, vpd.brks) {
  ...
  sDF$Photo_var[is.na(sDF$Photo_var)] <- 0.1152331
  mean(sDF$Cond_var,na.rm=TRUE)
  sDF$Cond_var[is.na(sDF$Cond_var)] <- 0.1932574
  mean(sDF$WUE_var,na.rm=TRUE)
  sDF$WUE_var[is.na(sDF$WUE_var)] <- 0.08729684
...
####### Using the SUBSET VPD range #####----------------------  
  ...
  SwueDF.EBF <- subset(SwueDF, PFT=="evergreen angiosperm")
  SwueDF.DBF <- subset(SwueDF, PFT=="deciduous angiosperm")
  SwueDF.ENF <- subset(SwueDF, PFT=="evergreen gymnosperm")
...
# Plot with CI shaded.
p6_PFT <- ggplot() +
  geom_ribbon(data=test_WUE_EBF,aes(x= X.VPDmean, ymin=ci.lb,ymax=ci.ub,fill="evergreen angiosperm"), alpha=0.1)+
  geom_ribbon(data=test_WUE_DBF,aes(x= X.VPDmean, ymin=ci.lb,ymax=ci.ub,fill="deciduous angiosperm"), alpha=0.1)+
  geom_ribbon(data=test_WUE_ENF,aes(x= X.VPDmean, ymin=ci.lb,ymax=ci.ub,fill="evergreen gymnosperm"), alpha=0.1)+
  geom_point(data=SwueDF,aes(x=VPDmean,y=WUE_resp,fill=PFT, colour= PFT, shape=Dataset, size=1/WUE_var))+
  ...
  stat_smooth(data=test_WUE_DBF,aes(x=X.VPDmean,y=pred,colour="deciduous angiosperm"),method="lm",formula=y~x,fullrange=T,size=1,se=FALSE)+
  stat_smooth(data=test_WUE_ENF,aes(x=X.VPDmean,y=pred,colour="evergreen gymnosperm"),method="lm",formula=y~x,fullrange=T,size=1,se=FALSE)+
  ...
  xlab("VPD (kPa)")+
  ylab("WUE response ratio (log_Resp / log_CO2)")+
  scale_colour_manual(values=c("evergreen angiosperm"="#009E73","deciduous angiosperm"="#0072B2","evergreen gymnosperm"="#D55E00"))+
  scale_fill_manual(values=c("evergreen angiosperm"="#009E73","deciduous angiosperm"="#0072B2","evergreen gymnosperm"="#D55E00"))+
  scale_shape_manual(values=c("B_Glencorse"=24,
                              "B_SCC"=22,
                              "BIFOR"=24,
                              "Duke FACE"=23,
                              "EucFACE"=24,
...
p7_PFT <- ggplot() +
  geom_ribbon(data=test_Cond_EBF,aes(x= X.VPDmean, ymin=ci.lb,ymax=ci.ub,fill="evergreen angiosperm"), alpha=0.1)+
  geom_ribbon(data=test_Cond_DBF,aes(x= X.VPDmean, ymin=ci.lb,ymax=ci.ub,fill="deciduous angiosperm"), alpha=0.1)+
  geom_ribbon(data=test_Cond_ENF,aes(x= X.VPDmean, ymin=ci.lb,ymax=ci.ub,fill="evergreen gymnosperm"), alpha=0.1)+
  geom_point(data=ScondDF,aes(x=VPDmean,y=Cond_resp,fill=PFT, colour= PFT, shape=Dataset, size=1/Cond_var))+
  ...
  scale_shape_manual(values=c("B_Glencorse"=24,
                              ...
                              "Rhinelander"=21,
                              "Richmond_WTC1"=22,
                              "Richmond_WTC2"=23));p7_PFT




p8_PFT <- ggplot() +
  geom_ribbon(data=test_Photo_EBF,aes(x= X.VPDmean, ymin=ci.lb,ymax=ci.ub,fill="evergreen angiosperm"), alpha=0.1)+
  geom_ribbon(data=test_Photo_DBF,aes(x= X.VPDmean, ymin=ci.lb,ymax=ci.ub,fill="deciduous angiosperm"), alpha=0.1)+
  geom_ribbon(data=test_Photo_ENF,aes(x= X.VPDmean, ymin=ci.lb,ymax=ci.ub,fill="evergreen gymnosperm"), alpha=0.1)+
  geom_point(data=SphotoDF,aes(x=VPDmean,y=Photo_resp,fill=PFT, colour= PFT, shape=Dataset, size=1/Photo_var))+
...
pdf(paste0(getwd(), "/output/CO2resp_PFT_Shape_colour_shaded.pdf"),
    width=7, height=15)
...
p6_PFT <- ggplot() +
  ...
  stat_smooth(data=test_WUE_EBF,aes(x=X.VPDmean,y=ci.lb,colour="evergreen angiosperm"),method="loess",size=1,linetype=3, se=FALSE)+
  stat_smooth(data=test_WUE_EBF,aes(x=X.VPDmean,y=ci.ub,colour="evergreen angiosperm"),method="loess", size=1,linetype=3,se=FALSE)+
  stat_smooth(data=test_WUE_DBF,aes(x=X.VPDmean,y=pred,colour="deciduous angiosperm"),method="lm",formula=y~x,fullrange=T,size=1,se=FALSE)+
  stat_smooth(data=test_WUE_DBF,aes(x=X.VPDmean,y=ci.lb,colour="deciduous angiosperm"),method="loess",size=1,linetype=3,se=FALSE)+
  stat_smooth(data=test_WUE_DBF,aes(x=X.VPDmean,y=ci.ub,colour="deciduous angiosperm"),method="loess",size=1,linetype=3,se=FALSE)+
  stat_smooth(data=test_WUE_ENF,aes(x=X.VPDmean,y=pred,colour="evergreen gymnosperm"),method="lm",formula=y~x,fullrange=T,size=1,se=FALSE)+
  stat_smooth(data=test_WUE_ENF,aes(x=X.VPDmean,y=ci.lb,colour="evergreen gymnosperm"),method="loess", size=1,linetype=3,se=FALSE)+
  stat_smooth(data=test_WUE_ENF,aes(x=X.VPDmean,y=ci.ub,colour="evergreen gymnosperm"),method="loess",size=1,linetype=3,se=FALSE)+
  geom_point(data=SwueDF,aes(x=VPDmean,y=WUE_resp,fill=PFT, colour= PFT, shape=Dataset, size=1/WUE_var))+
  geom_abline(slope=0, intercept=1, lty=4, colour = "grey")+
  theme_linedraw()+
  theme(panel.grid.minor=element_blank(),
...
pdf(paste0(getwd(), "/output/CO2resp_PFT_Shape_colour.pdf"),
    width=7, height=15)
plot_grid(combined_plot_PFT, legend_shared_PFT, ncol=1, rel_heights=c(1,0.15))
dev.off()  
